...
if NOT DEFINED LIBFFI_OUT set LIBFFI_OUT=%~dp0output
set _LIBFFI_OUT=%LIBFFI_OUT%\%ARCH%

echo get VS build environment
call %VCVARSALL% %VCVARS_PLATFORM% %SDK_VERSION%
...
echo ================================================================
%SH% -lc "(cd $OLDPWD; ./configure CC='%MSVCC% %ASSEMBLER% %BUILD_PDB% -DUSE_STATIC_RTL' CXX='%MSVCC% %ASSEMBLER% %BUILD_PDB% -DUSE_STATIC_RTL' LD='link' CPP='cl -nologo -EP' CXXCPP='cl -nologo -EP' CPPFLAGS='-DFFI_BUILDING' %BUILD_NOOPT% NM='dumpbin -symbols' STRIP=':' --build=$BUILD --host=$HOST;)"
if errorlevel 1 exit /B %ERRORLEVEL%
...
REM copy %ARTIFACTS%\.libs\libffi-*.dll %_LIBFFI_OUT% || exit /B 1
REM copy %ARTIFACTS%\.libs\libffi-*.lib %_LIBFFI_OUT% || exit /B 1
copy %ARTIFACTS%\.libs\libffi_convenience.lib %_LIBFFI_OUT% || exit /B 1
REM copy %ARTIFACTS%\.libs\libffi-*.pdb %_LIBFFI_OUT%
